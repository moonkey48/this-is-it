...
4. If no overlay appears, tell the user to reload the page or click the pinned Context Capture C icon while the command is still waiting. If it is not pinned, use Chrome toolbar puzzle icon > Context Capture. If Context Capture is not listed there, the extension is not loaded in the active Chrome profile.
